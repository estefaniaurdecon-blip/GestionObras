...
export const HistoryReportsPanel = ({
  ...
  useEffect(() => {
    setVisibleCount(HISTORY_REPORTS_PAGE_SIZE);
  }, [
  ...
  const visibleReports = useMemo(
    () => filteredHistoryReports.slice(0, visibleCount),
    [filteredHistoryReports, visibleCount],
  );
  const hiddenReportsCount = Math.max(filteredHistoryReports.length - visibleCount, 0);
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div className="text-sm text-muted-foreground">
          ...
          <span className="font-medium">{visibleReports.length}</span> de{' '}
              ...
              <Button
                ...
                onClick={() =>
                  setVisibleCount((current) =>
                    Math.min(current + HISTORY_REPORTS_PAGE_SIZE, filteredHistoryReports.length),
                  )
                }
              >
                Mostrar {Math.min(HISTORY_REPORTS_PAGE_SIZE, hiddenReportsCount)} mas
              </Button>
            </div>
          ) : null}
        </div>
