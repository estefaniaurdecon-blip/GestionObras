...
import {
  createWorkPostventa as createWorkPostventaApi,
  deleteWorkPostventa as deleteWorkPostventaApi,
  listWorkPostventas,
  type ApiWorkPostventa,
  updateWorkPostventa as updateWorkPostventaApi,
...
type StoredPostventa = WorkPostventa & {
  serverId?: number | null;
  syncStatus?: SyncStatus;
  lastSyncError?: string | null;
};

const STORAGE_KEY_PREFIX = 'work_postventas_local::v1::';
...
function toSubcontractGroups(value: unknown): PostventaSubcontractGroup[] {
...
function normalizeStoredPostventa(raw: unknown): StoredPostventa | null {
  ...
  const normalizedStatus: WorkPostventa['status'] =
...
function mapApiPostventa(item: ApiWorkPostventa): StoredPostventa {
...
function sortItems(items: StoredPostventa[]): StoredPostventa[] {
...
export const useWorkPostventas = (workId: string) => {
  ...
  const [postventasStored, setPostventasStored] = useState<StoredPostventa[]>([]);
  const [loading, setLoading] = useState(true);
  const storedRef = useRef<StoredPostventa[]>([]);

  useEffect(() => {
    storedRef.current = postventasStored;
  }, [postventasStored]);

  const saveLocal = useCallback(async (next: StoredPostventa[]) => {
    const sorted = sortItems(next);
    storedRef.current = sorted;
    setPostventasStored(sorted);
    await storage.setItem(storageKey, JSON.stringify(sorted));
  }, [storageKey]);

  const pushOne = useCallback(async (item: StoredPostventa): Promise<StoredPostventa | null> => {
    ...
    if (item.syncStatus === 'pending_delete') {
      ...
      try {
        await deleteWorkPostventaApi(item.serverId, tenantId);
        return null;
      } catch (error) {
    ...
    try {
      const response = item.serverId
        ? await updateWorkPostventaApi(item.serverId, payload, tenantId)
        : await createWorkPostventaApi(payload, tenantId);
      const synced = mapApiPostventa(response);
      synced.id = item.id;
  ...
  const syncPending = useCallback(async () => {
    ...
    const syncedItems: StoredPostventa[] = [];
  ...
  const refreshPostventas = useCallback(async () => {
    setLoading(true);
    try {
      ...
      const localItems = Array.isArray(parsed)
        ? parsed.map(normalizeStoredPostventa).filter((item): item is StoredPostventa => Boolean(item))
        : [];
      ...
      try {
        const remote = await listWorkPostventas({
          tenantId,
          projectId,
          includeDeleted: false,
          limit: 500,
        });
        const remoteItems = remote.map(mapApiPostventa);
        const map = new Map<string, StoredPostventa>();
  ...
  const createPostventa = useCallback(async (data: CreatePostventaData) => {
    ...
    const pending: StoredPostventa = {
      id: localId,
      work_id: workId,
      organization_id: String(organization?.id || ''),
      code: `POS-LOCAL-${new Date().getTime().toString().slice(-5)}`,
    ...
    if (synced) {
      ...
      if (synced.syncStatus === 'error') {
        toast.warning('Post-venta guardada en local. Pendiente de sincronizar.');
      }
      return synced;
    }
    return pending;
  ...
  const visiblePostventas = useMemo(
    () => postventasStored.filter((item) => item.syncStatus !== 'pending_delete').map((item) => ({
      ...item,
      organization_id: item.organization_id || String(organization?.id || ''),
    })),
    [organization?.id, postventasStored]
  );

  const stats = useMemo(() => ({
    total: visiblePostventas.length,
    pending: visiblePostventas.filter((r) => r.status === 'pending').length,
    inProgress: visiblePostventas.filter((r) => r.status === 'in_progress').length,
    completed: visiblePostventas.filter((r) => r.status === 'completed').length,
    totalEstimatedHours: visiblePostventas.reduce((sum, r) => sum + Number(r.estimated_hours || 0), 0),
    totalActualHours: visiblePostventas.reduce((sum, r) => sum + Number(r.actual_hours || 0), 0),
  }), [visiblePostventas]);

  return {
    postventas: visiblePostventas as WorkPostventa[],
    loading,
    stats,
    createPostventa,
    updatePostventa,
