-- Actualizar la función de notificación de partes completados para incluir la fecha
...
BEGIN
  ...
  IF OLD.status IS DISTINCT FROM NEW.status AND NEW.status = 'completed' AND OLD.status != 'completed' THEN
    ...
    SELECT DISTINCT ON (recipient_id)
           ...
           creator_name || ' ha completado el parte de ' || NEW.work_name || ' del día ' || formatted_date,
           NEW.id,
           NEW.organization_id
    FROM recipients
...
-- Actualizar la función de notificación de partes creados para incluir la fecha
...
BEGIN
  ...
  SELECT DISTINCT ON (recipient_id)
         ...
         creator_name || ' ha creado un nuevo parte de ' || NEW.work_name || ' del día ' || formatted_date,
         NEW.id,
         NEW.organization_id
  FROM recipients
  WHERE recipient_id IS NOT NULL
