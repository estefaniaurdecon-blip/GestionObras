-- Primero eliminar TODAS las políticas SELECT en work_reports
DO $$ 
DECLARE 
    r RECORD;
BEGIN
    FOR r IN 
        ...
        AND cmd = 'SELECT'
    LOOP
        EXECUTE format('DROP POLICY IF EXISTS %I ON public.work_reports', r.policyname);
    END LOOP;
END $$;

-- Crear nueva política que permite a site_managers ver solo partes completados
CREATE POLICY "view_reports_role_based"
