...
-- 1) Fusionar y eliminar duplicados en work_inventory
-- Usamos ROW_NUMBER() en lugar de MIN(id) para seleccionar el registro a conservar
WITH groups AS (
  SELECT 
    work_id,
    name,
...
-- 2) Crear constraint única para prevenir duplicados futuros
ALTER TABLE public.work_inventory
ADD CONSTRAINT work_inventory_unique_key
UNIQUE NULLS NOT DISTINCT (work_id, name, item_type, category, unit);
