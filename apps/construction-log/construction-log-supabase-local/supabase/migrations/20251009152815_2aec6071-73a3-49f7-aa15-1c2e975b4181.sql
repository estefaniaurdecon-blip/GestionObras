-- Fix duplicate notifications by using DISTINCT ON to ensure each user gets only one notification
-- Also add better logging to understand if notifications are being created
...
BEGIN
  ...
  -- Insert notifications for recipients, ensuring NO duplicates even if user has multiple roles
  WITH recipients AS (
