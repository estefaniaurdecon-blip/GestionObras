-- Corregir función de notificaciones para filtrar por organización
...
BEGIN
  ...
  -- Insert notifications for recipients ONLY in the same organization
  WITH recipients AS (
    ...
    WHERE ur.role = 'site_manager'
      ...
      AND ur.organization_id = NEW.organization_id  -- FILTRO POR ORGANIZACIÓN
    
    UNION
    
    -- Los admins reciben notificaciones SOLO de su organización
    SELECT DISTINCT ur.user_id AS recipient_id
    FROM public.user_roles ur
    WHERE ur.role = 'admin'
      AND ur.organization_id = NEW.organization_id  -- FILTRO POR ORGANIZACIÓN
...
BEGIN
  ...
  IF OLD.status IS DISTINCT FROM NEW.status AND NEW.status = 'completed' AND OLD.status != 'completed' THEN
    ...
    -- Notificar a site_managers SOLO de la misma organización
    WITH recipients AS (
      SELECT DISTINCT ur.user_id AS recipient_id
      FROM public.user_roles ur
      JOIN public.work_assignments wa ON wa.user_id = ur.user_id
